...
/*   Updated: 2026/07/01 11:31:11 by guillsan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_redirs(t_cmd *cmd)
{
	...
	while (redir)
	{
		if (redir->type == REDIR_IN)
			ret_code = handle_redir_in(redir);
		else if (redir->type == REDIR_OUT)
			ret_code = handle_redir_out(redir);
		else if (redir->type == APPEND)
			ret_code = handle_redir_append(redir);
		else if (redir->type == HEREDOC)
			ret_code = handle_redir_heredoc(redir);
